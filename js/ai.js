...
export function aiSystem(world, dt) {
    world.entities.forEach(entity => {
        if (entity.id === state.playerId || !entity.ai || !entity.alive || entity.fainted) return;
...
function ghostAI(entity, world, dt) {
    const trackers = world.entities.filter(e => e.role === 'tracker' && !e.fainted);
...
function trackerAI(entity, world, dt) {
    ...
    const faintedAlly = world.entities.find(e => e.role === 'tracker' && e.fainted && e !== entity);
    ...
    entity.flashlightActive = (target === ghost && len < 250) || (target === faintedAlly && len < 100);
}
